...
module.exports = function(metric) {
  ...
  function * savePkg(pkg) {
    ...
    yield storage.writeContent(path.join(config.directory, `/packages/${pkg.name}`), data);
  }
  ...
  function * upload(pkg) {
    ...
    for (let filename of Object.keys(attachments)) {
      ...
      yield storage.writePackage(path.join(config.directory, `/tarballs/${pkg.name}/${filename}/${sha}${ext}`), data);
    }
    yield savePkg(pkg);
  }
