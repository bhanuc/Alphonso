...
module.exports = function(metric) {
  ...
  function * savePkg(pkg) {
    ...
    yield fs.writeContent(path.join(config.directory, `/packages/${pkg.name}`), data);
  }

  function refreshPkg(npmPkg) {
    co(function * () {
      let localPkg = yield fs.download(`/packages/${npmPkg.name}`);
      if (!localPkg) {
  ...
  function * get(name, etag) {
    ...
    if (pkg === 404) {
      pkg = yield fs.download(`/packages/${name}`);
      if (!pkg) {
        return 404;
      }
  ...
  function * upload(pkg) {
    ...
    for (let filename of Object.keys(attachments)) {
      ...
      yield fs.writePackage(path.join(config.directory, `/tarballs/${pkg.name}/${filename}/${sha}${ext}`), data);
    }
    yield savePkg(pkg);
  }
