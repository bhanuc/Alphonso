...
function * createAuthToken(username) {
  ...
  yield storage.writePackage(path.join(config.directory, '/auth_tokens'), new Buffer(JSON.stringify(creds)), {
...
function * authenticate(user) {
  let creds = (yield storage.download(path.join(config.directory,'/htpasswd'))) || '';
