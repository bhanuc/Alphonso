...
app.use(r.get('/:scope/:name/-/:filename/:sha', function * (scope, name, filename, sha) {
  ...
  let tarball = yield tarballs(this.metric, this).get(`${scope}/${name}`, filename, sha);
  if (!tarball) {
    this.status = 404;
    this.body = {
      error: 'no tarball found'
...
// get package tarball with sha 
app.use(r.get('/:name/-/:nameversion/:sha/:filename', function * (name, nameversion, sha, filename) {
  let ext = path.extname(filename);
  // filename = path.basename(filename, ext);
  this.redirect(`/${name}/-/${nameversion}${ext}/${sha}`);
}));
